...
echo 0 > /sys/kernel/msm_performance/parameters/splh_log_level
...
#Limit the dmabuf pool for zizhan
vendor_name=`getprop ro.product.vendor.name`
case "$vendor_name" in
	"zizhan")
		echo 120000 > /sys/kernel/dma_mi_pool/msm_total_pools_max
	;;
	*)
	;;
esac
